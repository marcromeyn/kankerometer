...
alfaTemp = zeros(numberOfStates, 1); %alfaTemp = zeros(numberOfStates); 
for t=2:T
    
    for j=1:numberOfStates
        alfaTemp(j) = B(j,t)*(sum(alfaHat(:,t-1)'*A(:,j)));
    end
    c(t) = sum(alfaTemp); % c(t,1) = sum(alfaTemp);
    for j=1:numberOfStates
        alfaTemp(j) = alfaTemp(j)/c(t); % alfaTemp(j) = alfaTemp(j)/c(2); 
%         cz = sign(randn(1))*cz; 
    end
    alfaHat = [alfaHat alfaTemp]; % alfaHat = [alfaHat alfaTemp'];
end
[rows,columns] = size(A);
if(rows ~= columns)
     c(max(rows,columns)+1) = alfaTemp'*A(:,columns); % 0.0581; 
end
